...
-(void)dropCargo
{
    ...
    if (hasDroppedMallet == NO)
    {
        ...
        [delegate createObjectOfType:kPowerUpTypeMallet withHealth:0.0f
                          atLocation:cargoDropPosition withZValue:50];
    }
    else
    {
        CCLOG(@"SpaceCargoShip --> Health Powerup was created!");
        [delegate createObjectOfType:kPowerUpTypeHealth withHealth:0.0f
                          atLocation:cargoDropPosition withZValue:50];
    }
}
    ...
    if(self=[super initWithSpriteFrameName:spriteFrameName])
    {
        ...
        id action = [CCRepeatForever actionWithAction:
                     [CCSequence actions:
                      ...
                      [CCMoveTo actionWithDuration:0.01f
                                          position:position1],
                      ...
                      [CCCallFunc actionWithTarget:
                       self selector:@selector(dropCargo)],
                      [CCMoveTo actionWithDuration:0.0f
                                          position:offScreen],
                      nil]
                     ];
